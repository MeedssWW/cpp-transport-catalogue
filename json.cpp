...
bool Node::operator==(const Array& rhs) const {
    if (!IsArray()) return false;
    return AsArray() == rhs;
}

bool Node::operator==(const Dict& rhs) const {
    if (!IsMap()) return false;
    return AsMap() == rhs;
}

// Реализации методов Document
